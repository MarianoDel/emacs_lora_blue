...
#define SX_RST_ON    PB0_OFF
#define SX_RST_OFF    PB0_ON
...
unsigned char SxFskInit (void)
{
    ...
    // set bitrate
    SxBaseWrite(REG_BITRATEMSB, SRF_PARAMP_MODULATIONSHAPING_01);
    SxBaseWrite(REG_BITRATELSB, SRF_PARAMP_MODULATIONSHAPING_01);

    // set freq deviation
    SxBaseWrite(REG_FDEVMSB, SRF_PARAMP_MODULATIONSHAPING_01);
    SxBaseWrite(REG_FDEVLSB, SRF_PARAMP_MODULATIONSHAPING_01);

    // set freq
    SxBaseWrite(REG_FRFMSB, SRF_PARAMP_MODULATIONSHAPING_01);
    SxBaseWrite(REG_FRFMID, SRF_PARAMP_MODULATIONSHAPING_01);
    SxBaseWrite(REG_FRFLSB, SRF_PARAMP_MODULATIONSHAPING_01);

    // transmitter settings
    // set power output
    SxBaseWrite(REG_PACONFIG, SRF_PARAMP_MODULATIONSHAPING_01);

    // set ramp and modulation shaping Gaussian BT = 1
    SxBaseWrite(REG_PARAMP, SRF_PARAMP_MODULATIONSHAPING_01);

    // set overload current
    SxBaseWrite(REG_OCP, SRF_PARAMP_MODULATIONSHAPING_01);
